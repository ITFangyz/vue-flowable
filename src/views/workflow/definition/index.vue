  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" fit :data="definitionList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="250">
        <template slot-scope="scope">
          <el-button type="text" size="mini" icon="el-icon-edit" @click="handleDesigner(scope.row)" >流程设计</el-button>
          <el-button type="text" size="mini" icon="el-icon-connection" v-if="scope.row.formId == null"
            @click.native="handleAddForm(scope.row)">配置表单</el-button>
          <el-button type="text" size="mini" icon="el-icon-delete" @click="handleDelete(scope.row)" >流程删除</el-button>
          <!-- <el-button type="text" size="mini" icon="el-icon-price-tag" @click.native="handlePublish(scope.row)">版本管理</el-button> -->
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="版本管理" :visible.sync="publish.open" width="50%" append-to-body>
      <el-table v-loading="publish.loading" :data="publish.dataList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <el-button type="text" size="mini" icon="el-icon-video-pause" v-if="!scope.row.suspended"
              @click.native="handleUpdateSuspended(scope.row)" >挂起
            </el-button>
            <el-button type="text" size="mini" icon="el-icon-video-play" v-if="scope.row.suspended"
              @click.native="handleUpdateSuspended(scope.row)" >激活
            </el-button>
            <el-button type="text" size="mini" icon="el-icon-delete" @click="handleDelete(scope.row)" >删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <pagination v-show="publishTotal > 0" :total="publishTotal" :page.sync="publishQueryParams.pageNum"
        :limit.sync="publishQueryParams.pageSize" @pagination="getPublishList" />
    </el-dialog>
    ...
    <designer v-if="isDesignerShow" :designerForm="processForm" @save="submitSave()" @close="isDesignerShow = false">
    </designer>
